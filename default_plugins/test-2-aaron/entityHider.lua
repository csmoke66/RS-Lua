...
hider = {
    ...
    updateLoop = function(self)
        -- Get Local Player
        _, Local = osrs.ClientOp.p_findself();

        -- npcs
        npcs = osrs.ClientOp.getnpc_all()
        for index, npc_id in pairs(npcs) do
            npc = osrs.ClientOp.getnpc_obj(npc_id)
            npc:render3D(self:renderNPC3D(npc))
            npc:render2D(not self.isHiddenNPC2D)
        end

        -- players
        players = osrs.ClientOp.getplayers_all()
        for index, player_id in pairs(players) do
            player = osrs.ClientOp.getplayer_obj(player_id)
            player:render3D(self:renderPlayer3D(player))
            player:render2D(self:renderPlayer2D(player))
        end

        -- projectiles 
        osrs.ClientOp.render_projectiles(not self.isHiddenProjectiles)
    end,
